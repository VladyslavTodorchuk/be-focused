class Goal < ApplicationRecord
	 ...
	 validates :title, presence: true, length: {maximum: 80}, format: { with: /([a-zA-Z-0-9]|[!#$%&'*+-\/=?^_`{|}~. ])./, message: "Please enter title in correct format"}
	 validates :body, presence: true, length: {maximum: 500}, format: { with: /([a-zA-Z-0-9]||[!#$%&'*+-\/=?^_`{|}~. ])./, message: "Please enter body in correct format"}
